...
#用法说明
def _usage():
    print('''spider  --littlethunder
用法：python spider.py -u [URL] -d [Deep] -f [Log File]\
 -l [Level] --concurrency [Thread Number] --dbfile \
[Database File Name] --key [Key Word]

-h  帮助
-u  [URL] （必选参数）指定爬虫开始地址，必须以http或https开头
-d  [Deep] 指定爬虫深度，默认为1，即为当前页
-f  [Log File] 日志文件路径，默认本地目录下spider.log
-l  [Level] 日志记录详细程度，数字越大越详细，1-5取值，默认1
--concurrency  [Thread Number] 指定线程池大小，默认1
